...
- [ ] `setLevel(level: 1 | 2 | 3 | 4 | 5 | 6): this` --- изменяет уровень заголовка.
      **TODO:** метод требует доработки. Сейчас он заменяет DOM-элемент заголовка, и после этого
      внутренние менеджеры компонента могут остаться привязаны к старому элементу. До исправления
...
const features = new ListComponent({
  type: 'ul',
